...
def activar_tenant(nombre_schema):
    ...
    try:
        ...
        print(f"✓ Tenant '{nombre_schema}' activado")
        return empresa
    except Empresa.DoesNotExist:
        print(f"❌ No se encontró el tenant '{nombre_schema}'")
        return None
...
def generar_marcas():
    ...
    print(f"✓ {len(marcas)} marcas creadas + 1 genérica")
...
def generar_ventas(variantes, clientes):
    ...
    print(f"✓ {len(ventas)} ventas creadas con sus detalles")
    return ventas
...
def generar_compras(variantes, proveedores):
    ...
    print(f"✓ {len(compras)} compras creadas con sus detalles")
    return compras
...
def run():
    ...
    if not empresa:
        print("\n❌ No se pudo conectar al tenant. Asegúrate de que 'tienda_amiga' exista.")
...
🔑 Usuarios para login (password: password123):
